...
const parseVintedURL = (search_text, order = "newest_first", domain = 'be', catalog_ids = "", color_ids = "", brand_ids= "", size_ids = "", material_ids = "", video_game_rating_ids = "", status_ids = "", is_for_swap = 0, page = 1, per_page = 24) => {
 ...
 */
const vintedSearch = (search_text, order = "newest_first") => {
    return new Promise(async (resolve, reject) =>{
        var c = cookies.get('be') || await fetchCookie('be')
        const controller = new AbortController();
        fetch(parseVintedURL(search_text, order), {
            headers: {
